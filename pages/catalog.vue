                    ...
                    <b-col xl="9" lg="9">
                        <div class="catalog__filter-tags">
                            <g-tag
                                ...
                                :label="getFilterNameForTag({ filterType: 'categories', filter: item }).name"
                                closable
                                :key="item"
                                :value="item"
                            ...
                            <g-tag
                                ...
                                class="catalog__filter-tag"
                                :label="getFilterNameForTag({ filterType: 'platforms', filter: item }).name"
                                closable
                                :key="item"
                                :value="item"
                            ...
                            <g-tag
                                ...
                                class="catalog__filter-tag"
                                :label="getFilterNameForTag({ filterType: 'genres', filter: item }).name"
                                closable
                                :key="item"
                                :value="item"
                                filter-type="genres"
                            ...
                            <g-tag
                                ...
                                class="catalog__filter-tag"
                                :label="getFilterNameForTag({ filterType: 'services', filter: item }).name"
                                closable
                                :key="item"
                                :value="item"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            isClearTagsButtonShow () {
                ...
                for (let item in this.acceptedFilters) {
                    ...
                    if (typeof filterGroup === 'object' && 'length' in filterGroup && filterGroup.length > 0) {
                        isShow = true
                        break
                    }
                }
            ...
            minPrice: {
                ...
                set (val) {
                    this.$store.dispatch(
                        'catalog/changePriceFilter', 
                        { minPrice: Number(val) * 100, maxPrice: null }
                        )
                }
            },
            maxPrice: {
                ...
                set (val) {
                    this.$store.dispatch(
                        'catalog/changePriceFilter', 
                        { minPrice: null, maxPrice: Number(val) * 100 }
                        )
                }
            }
        },
        data: () => {
            return {
                filtersStrings: ['minPrice', 'maxPrice', 'categories', 'genres', 'platforms', 'services'],
                showPopup: false,
                priceRange: [0, 0],
                categoriesToShow: 2,
                genresToShow: 5,
        ...
        methods: {
            ...
            async checkQueryString () {
              ...
              for (let key in this.filtersStrings) {
                  let item = this.filtersStrings[key]
                  if (item in query) {
                      await this.$store.dispatch('catalog/addFilter', { filterType: item, filter: query[item] })
                      data.push(item)
                  }
              }
